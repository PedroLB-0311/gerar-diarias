...
export async function gerarPDF(form, logoUrl = "") {
  ...
  if (logoBase64) {
    content.push({
      columns: [
        { image: logoBase64, width: 180, alignment: "left" },
        { width: "*", text: "" },
      ],
      margin: [0, 0, 0, 10],
    });
  ...
  if (form.cpf) content.push({ text: `CPF: ${form.cpf}`, style: "field" });
  ...
  form.trips.forEach((trip, index) => {
    ...
    if (trip.totalDiaria !== undefined) content.push({ text: `Diária: R$ ${trip.totalDiaria.toFixed(2)}`, style: "field" });
    if (trip.totalPernoite !== undefined) content.push({ text: `Pernoite: R$ ${trip.totalPernoite.toFixed(2)}`, style: "field" });
    if (trip.totalDiaria !== undefined || trip.totalPernoite !== undefined) {
      const total = (trip.totalDiaria || 0) + (trip.totalPernoite || 0);
      content.push({ text: `Valor Total: R$ ${total.toFixed(2)}`, style: "field", margin: [0, 0, 0, 10] });
    }

...
export async function gerarPDF(form, logoUrl = "") {
  ...
  content.push({
    text: "Nos Termos do Decreto n. 56/2025",
    alignment: "center",
    style: "subheader",
    margin: [0, 0, 0, 5],
  });
  ...
  content.push({
    stack: [
      { text: `Servidor: ${form.servidor || "____________________________"}`, style: "field" },
      { text: `CPF: ${form.cpf || "____________________________"}`, style: "field" },
      { text: `Cargo: ${form.cargo || "____________________________"}`, style: "field" },
      { text: `Matrícula: ${form.matricula || "____________________________"}`, style: "field" },
      { 
        text: `Grupo de Diária: (${form.grupo === "A" ? "x" : " "}) A   (${form.grupo === "B" ? "x" : " "}) B   (${form.grupo === "B Acompanhando A" ? "x" : " "}) B Acompanhando A`, 
        style: "field" 
      },
    ],
    margin: [0, 2, 0, 5],
  ...
  form.trips.forEach((trip) => {
    ...
    content.push({
      stack: [
        { 
          text: form.trips[0]?.destinos && form.trips[0].destinos.length > 0
            ? `Destino: ${form.trips[0].destinos.map(d => `${d.nome}${d.uf ? ` (${d.uf})` : ""}`).join(" / ") || "____________________________"}`
            : `Destino: ____________________________`,
          style: "field"
        },
        { 
          text: `(${form.trips[0]?.distanciaKm < 200 ? "x" : " "}) Inferior a 200km   (${form.trips[0]?.distanciaKm >= 200 ? "x" : " "}) Superior a 200km`, 
          style: "field" 
        },
        ...(trip.saidas && trip.saidas.length > 0 ? trip.saidas.map(saida => [
          {
            text: `Veículo: (${saida.veiculo?.tipo === "Oficial" ? "x" : " "}) Oficial   (${saida.veiculo?.tipo === "Particular" ? "x" : " "}) Particular   Placa: ${saida.veiculo?.placa || "____________________________"}`,
            style: "field",
          },
          {
            text: `Data de Saída: ${formatDate(saida.diaSaida) || formatDate(trip.diaSaida) || "____________"} ----- Hora de Saída: ${saida.horaSaida || "____:____"}`,
            style: "field",
          },
          {
            text: `Data de Retorno: ${formatDate(saida.diaRetorno) || formatDate(trip.diaRetorno) || "____________"} ----- Hora de Retorno: ${saida.horaRetorno || "____:____"}`,
            style: "field",
          }
        ]).flat() : []),
      ],
    ...
    const cleanTipos = tiposDetalhados.map(t => t.replace(/ \(.+\)$/, ""));
    const diaria04_08 = cleanTipos.includes("Entre 4 e 8 horas");
    const diariaAcima08 = cleanTipos.includes("Mais de 8 horas");
    const outroEstado = cleanTipos.includes("Outro Estado");
    ...
    content.push({
      text: `(${outroEstado ? "x" : " "}) Outro Estado – Afastamento acima de 08 horas     (${comPernoite ? "x" : " "}) Com pernoite`,
      style: "field",
      margin: [0, 2, 0, 5],
    });
    ...
    content.push({
      text: `Diária: R$ ${trip.totalDiaria?.toFixed(2) || "____________"}`,
      style: "field",
      margin: [0, 2, 0, 2],
    });
    content.push({
      text: `Pernoite: R$ ${trip.totalPernoite?.toFixed(2) || "____________"}`,
      style: "field",
      margin: [0, 2, 0, 2],
    });
    const total = (trip.totalDiaria || 0) + (trip.totalPernoite || 0);
    content.push({
      text: `Valor Total: R$ ${total.toFixed(2) || "____________"}`,
      style: "field",
      margin: [0, 2, 0, 5],
    });
  ...
  // Assinatura do Servidor (Centered)
  content.push({
    columns: [
      { width: "*", text: "" },
      {
        ...
        stack: [
          ...
          { text: form.servidor || "____________________________", style: "field", alignment: "center" },
          { text: form.cpf || "____________________________", style: "field", alignment: "center" },
        ],
      },
      { width: "*", text: "" },
    ],
  ...
  // Assinatura do Secretário (Centered)
  content.push({
    columns: [
      { width: "*", text: "" },
      {
        ...
        stack: [
          { text: "________________________________________", style: "field", alignment: "center" },
          { text: form.secretario || "____________________________", style: "field", alignment: "center" },
          { text: form.secretaria || "____________________________", style: "field", alignment: "center" },
        ],
      },
      { width: "*", text: "" },
    ],
  ...
  // Data com fonte reduzida
  content.push({
    text: `São Ludgero-SC, ${new Date().getDate()} de ${new Date().toLocaleString("pt-BR", { month: "long" })} de ${new Date().getFullYear()}`,
    style: "smallFooter",
    alignment: "right",
    margin: [0, 5, 0, 0],
  });
  ...
  const docDefinition = {
    ...
    styles: {
      ...
      sectionTitle: { fontSize: 11, bold: true, decoration: "underline" },
      field: { fontSize: 10, margin: [0, 2, 0, 2] },
      smallFooter: { fontSize: 9, italics: true },
    },
  };
